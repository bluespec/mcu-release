...
PLI_INT32 socket_accept_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  int arg_1;
  int vpi_result;
  ...
  vpi_result = socket_accept(arg_1);
...
PLI_INT32 socket_accept_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}
...
void socket_accept_vpi_register()
{
  ...
  tf_data.tfname = "$socket_accept";
  tf_data.calltf = socket_accept_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = socket_accept_sizetf;
  tf_data.user_data = "$socket_accept";
