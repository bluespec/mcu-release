...
CATALYST_DIRS = $(CATALYST_RUNDIR)/src/bsv:$(CATALYST_INSTALL)/src/bsv/catalyst:$(CATALYST_INSTALL)/src/bsv/cxi/src
...
SYS_TOP_FILE   = $(CORE_REPO)/Sys/BSRISCVTop.bsv
...
compile_core: build_dir Core_RTL
	@echo '-------------------------'
	date +"(%F %T) Generating MCU Core (bsriscvtop) RTL ..."
...
compile_sim: build_dir Sim_RTL
	...
	$(BSC) -u -elab -verilog -vdir TB_RTL $(RTL_BDIRS) $(BSC_COMPILATION_FLAGS) -D WATCH_TOHOST -D TEST_GPIO -D TEST_UART $(BSC_PATH) $(SIM_TOP_FILE)
	rm TB_RTL/mkShifter_Box.v
	cp $(REPO)/src_Testbench/common/src_verilog/* TB_RTL/
	date +"(%F %T) Generated RTL into TB_RTL"
	@echo '-------------------------'
